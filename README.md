# odin4vim
A Vim and Neovim plugin to give native-feeling support for the [Odin](https://odin-lang.org/) programming language. Features include: syntax highlighting, proper indenting, quickfix error handling, and `[[`/`]]` to jump between functions.
...
- Manual: clone the repo into your vim config plugin folder
...
Run `:compiler! odin` to set `'makeprg'` to `odin build .` for that session. Then, running `:make` will build the current directory and fill the quickfix list with any warnings or errors, which can be quickly navigated with `[q`/`]q`.
...
Yes, now there are at least 4 separate odin plugins for vim.
